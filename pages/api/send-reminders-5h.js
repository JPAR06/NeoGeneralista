...
// Called hourly. Sends a same-day reminder to everyone with a confirmed
// reservation for events happening within the next 4-6 hours.
    ...
    for (const reserva of reservas) {
      try {
        const subject = cleanSubject(
          `Hoje — ${evento.edicao ?? "Algoritmo Humano"} começa em poucas horas`
        );
        const tplOpts = buildReminderTemplate({ reserva, evento, window: "5h" });
        await sendEmail({
          to: reserva.email,
          toName: reserva.nome,
          subject,
