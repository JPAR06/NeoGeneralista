...
//   - RFC 8058 one-click unsubscribe (HTTPS URL + List-Unsubscribe-Post) when
//     we have a recipient email to sign — Gmail's bulk-sender rules require
//     this for >5k/day senders and reward it for everyone else.
//   - Mailto fallback so clients that don't support RFC 8058 still work.
//   - Feedback-ID lets Gmail group sends and report per-stream spam rates
//     (visible in Postmaster Tools).
//   - Auto-Submitted: auto-generated for cron-triggered mail (RFC 3834).
//
// Reply-To is NOT here: Sender.net rejects it as a custom header
// (validation error 422). Configure Reply-To at the account level in the
// Sender.net dashboard instead — it then ships automatically on every send.
export function defaultHeaders({ topic = "geral", recipientEmail, autoSubmitted } = {}) {
  const headers = {
    "Feedback-ID": `${topic}:neogeneralista:transactional`,
  };
  ...
  if (recipientEmail) {
    ...
    headers["List-Unsubscribe"] = `<${httpsUrl}>, <mailto:${BRAND.contactEmail}?subject=unsubscribe-${topic}>`;
    headers["List-Unsubscribe-Post"] = "List-Unsubscribe=One-Click";
  } else {
    headers["List-Unsubscribe"] = `<mailto:${BRAND.contactEmail}?subject=unsubscribe-${topic}>`;
  }

  if (autoSubmitted) headers["Auto-Submitted"] = "auto-generated";
 ...
 * @param {string} [opts.subheading] Optional supporting line below heading
 * @param {string} [opts.greeting]   "Olá, Nome." — HTML allowed
 * @param {string|string[]} [opts.paragraphs]  Body paragraphs (HTML allowed)
 * @param {string} [opts.heroImage]  Optional banner image URL (used for blog notification)
 ...
 * @param {Array<{label: string, url: string, variant?: 'primary'|'secondary'}>} [opts.actions]
 *        Secondary buttons (e.g. calendar links)
 * @param {string} [opts.note]       Small grey note paragraph below CTA
 * @param {string} [opts.footerReason] "Recebeste este email porque…" line
...
export function renderEmail(opts) {
  ...
  const actionsHtml = (actions || [])
    .map((a) => {
      ...
      const bg = primary ? t.ctaBg : t.secondaryBtnBg;
      const color = primary ? t.ctaColor : t.secondaryBtnText;
      const border = primary ? "none" : `1px solid ${t.secondaryBtnBorder}`;
...
<tr><td style="padding:${heroImage ? "28px 36px 8px" : "32px 36px 8px"}">
...
export function renderEmailText(opts) {
  const { eyebrow, heading, subheading, greeting, paragraphs, details, cta, actions, note, footerReason, transactional } = opts;
  const lines = [];
